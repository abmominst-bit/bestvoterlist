...
export default function VoterSlipModal({
  ...
  const candidateName = (unionName && localStorage.getItem(`candidate_name_for_union_${unionName}`)) || 
                        localStorage.getItem('candidate_name') || 
                        'মোঃ রেজাউল করিম (মুকুল)';

  const candidatePhoto = (unionName && localStorage.getItem(`candidate_photo_for_union_${unionName}`)) || 
                         localStorage.getItem('candidate_photo') || 
                         '/mukul.png';

  const candidateSymbol = (unionName && localStorage.getItem(`candidate_symbol_for_union_${unionName}`)) || 
                          localStorage.getItem('candidate_symbol') || 
                          '/marka.png';

  const candidateSymbolName = (unionName && localStorage.getItem(`candidate_symbol_name_for_union_${unionName}`)) || 
                              localStorage.getItem('candidate_symbol_name') || 
                              'মোরগ';

  const candidateSlogan = (unionName && localStorage.getItem(`candidate_slogan_for_union_${unionName}`)) || 
                          localStorage.getItem('candidate_slogan') || 
                          'আমাকে আপনার মূল্যবান ভোট দিয়ে জয়যুক্ত করুন।';
