...
export default function AdminSettings({ settings, onUpdateSettings, unions }: AdminSettingsProps) {
  ...
  // Maintenance custom text & image states (saved in localStorage to avoid database changes)
  const [mTitle, setMTitle] = useState(() => localStorage.getItem('maintenance_title') || 'সিস্টেম রক্ষণাবেক্ষণ চলছে');
  const [mDesc, setMDesc] = useState(() => localStorage.getItem('maintenance_desc') || 'প্রিয় নাগরিক, ডিজিটাল ভোটার ডিরেক্টরি পোর্টালটি বর্তমানে সিস্টেম রক্ষণাবেক্ষণ কাজের জন্য সাময়িকভাবে বন্ধ রয়েছে। অনুগ্রহ করে কিছুক্ষণ পর পুনরায় চেষ্টা করুন।');
  const [mImage, setMImage] = useState(() => localStorage.getItem('maintenance_image') || '');

  // Candidate configuration states (saved in localStorage)
  const [candidateName, setCandidateName] = useState(() => localStorage.getItem('candidate_name') || 'মোঃ রেজাউল করিম (মুকুল)');
  const [candidateSlogan, setCandidateSlogan] = useState(() => localStorage.getItem('candidate_slogan') || 'আমাকে আপনার মূল্যবান ভোট দিয়ে জয়যুক্ত করুন।');
  const [candidatePhoto, setCandidatePhoto] = useState(() => localStorage.getItem('candidate_photo') || '');
  const [candidateSymbol, setCandidateSymbol] = useState(() => localStorage.getItem('candidate_symbol') || '');
  const [candidateSymbolName, setCandidateSymbolName] = useState(() => localStorage.getItem('candidate_symbol_name') || 'মোরগ');
  ...
  const [gscToken, setGscToken] = useState(() => localStorage.getItem('google_search_console_token') || '');

  // Synchronize Union-wise candidate configuration states whenever selectedUnion changes
  useEffect(() => {
    if (selectedUnion) {
      setUnionCandidateName(localStorage.getItem(`candidate_name_for_union_${selectedUnion}`) || '');
      setUnionCandidateSlogan(localStorage.getItem(`candidate_slogan_for_union_${selectedUnion}`) || '');
      setUnionCandidatePhoto(localStorage.getItem(`candidate_photo_for_union_${selectedUnion}`) || '');
      setUnionCandidateSymbol(localStorage.getItem(`candidate_symbol_for_union_${selectedUnion}`) || '');
      setUnionCandidateSymbolName(localStorage.getItem(`candidate_symbol_name_for_union_${selectedUnion}`) || '');
    } else {
  ...
  const handleSaveCustomContent = () => {
    localStorage.setItem('maintenance_title', mTitle);
    localStorage.setItem('maintenance_desc', mDesc);
    if (mImage) {
      localStorage.setItem('maintenance_image', mImage);
    } else {
      localStorage.removeItem('maintenance_image');
    }
    setSuccess('Maintenance custom text and image saved successfully in LocalStorage!');
  ...
  const handleSaveCandidateSettings = () => {
    ...
    localStorage.setItem('candidate_slogan', candidateSlogan);
    localStorage.setItem('candidate_symbol_name', candidateSymbolName);
    if (candidatePhoto) {
      localStorage.setItem('candidate_photo', candidatePhoto);
    } else {
      localStorage.removeItem('candidate_photo');
    }
    if (candidateSymbol) {
      localStorage.setItem('candidate_symbol', candidateSymbol);
    } else {
      localStorage.removeItem('candidate_symbol');
    }
    setSuccess('নির্বাচনী প্রার্থীর তথ্য সফলভাবে সংরক্ষিত হয়েছে!');
    setTimeout(() => setSuccess(''), 3000);
  };
  ...
  const handleSaveUnionCandidateSettings = () => {
    ...
    // Check if any override is actually provided, if yes save, else remove to prevent cluttering
    if (unionCandidateName.trim()) {
      localStorage.setItem(`candidate_name_for_union_${selectedUnion}`, unionCandidateName.trim());
    } else {
      localStorage.removeItem(`candidate_name_for_union_${selectedUnion}`);
    }

    if (unionCandidateSlogan.trim()) {
      localStorage.setItem(`candidate_slogan_for_union_${selectedUnion}`, unionCandidateSlogan.trim());
    } else {
      localStorage.removeItem(`candidate_slogan_for_union_${selectedUnion}`);
    }

    if (unionCandidateSymbolName.trim()) {
      localStorage.setItem(`candidate_symbol_name_for_union_${selectedUnion}`, unionCandidateSymbolName.trim());
    } else {
      localStorage.removeItem(`candidate_symbol_name_for_union_${selectedUnion}`);
    }

    if (unionCandidatePhoto) {
      localStorage.setItem(`candidate_photo_for_union_${selectedUnion}`, unionCandidatePhoto);
    } else {
      localStorage.removeItem(`candidate_photo_for_union_${selectedUnion}`);
    }

    if (unionCandidateSymbol) {
      localStorage.setItem(`candidate_symbol_for_union_${selectedUnion}`, unionCandidateSymbol);
    } else {
      localStorage.removeItem(`candidate_symbol_for_union_${selectedUnion}`);
    }

    // Refresh states
    setSuccess(`${selectedUnion} ইউনিয়নের নির্বাচনী প্রার্থীর তথ্য সফলভাবে সংরক্ষিত হয়েছে!`);
    setTimeout(() => setSuccess(''), 3000);
  };

  const handleResetUnionCandidateSettings = () => {
    if (!selectedUnion) return;
    if (window.confirm(`${selectedUnion} ইউনিয়নের সব কাস্টম প্রার্থীর তথ্য মুছে ফেলে গ্লোবাল ডিফল্ট প্রার্থীর তথ্য সচল করতে চান?`)) {
      localStorage.removeItem(`candidate_name_for_union_${selectedUnion}`);
      localStorage.removeItem(`candidate_slogan_for_union_${selectedUnion}`);
      localStorage.removeItem(`candidate_symbol_name_for_union_${selectedUnion}`);
      localStorage.removeItem(`candidate_photo_for_union_${selectedUnion}`);
      ...
      setSuccess(`${selectedUnion} ইউনিয়নের কাস্টম প্রার্থীর তথ্য মুছে ফেলা হয়েছে!`);
      setTimeout(() => setSuccess(''), 3000);
    }
  };

  const handleResetCandidateDefaults = () => {
    if (window.confirm('আপনি কি প্রার্থীর তথ্য রিসেট করতে চান?')) {
      localStorage.removeItem('candidate_name');
      localStorage.removeItem('candidate_slogan');
      localStorage.removeItem('candidate_photo');
      localStorage.removeItem('candidate_symbol');
      localStorage.removeItem('candidate_symbol_name');
  ...
  const handleSaveGSCSettings = () => {
    let token = gscToken.trim();
    // Parse if user pasted the full HTML meta tag
    ...
    localStorage.setItem('google_search_console_token', token);
    setGscToken(token);

    // Dynamic application to current DOM
    ...
    setSuccess('গুগল সার্চ কনসোল ভেরিফিকেশন কোড সফলভাবে সংরক্ষিত হয়েছে!');
    setTimeout(() => setSuccess(''), 3000);
  };

  const handleResetDefaults = () => {
    if (window.confirm('Are you sure you want to reset maintenance text & image to defaults?')) {
      localStorage.removeItem('maintenance_title');
      localStorage.removeItem('maintenance_desc');
      localStorage.removeItem('maintenance_image');
      ...
      setMImage('');
      setSuccess('Maintenance configuration reset to defaults!');
      setTimeout(() => setSuccess(''), 3000);
    }
  };
              ...
              <div className="pt-4 border-t border-slate-100 space-y-4 animate-fade-in">
                <div className="bg-amber-50/50 p-3.5 rounded-xl border border-amber-100 text-[11px] text-amber-800 font-bold space-y-1">
                  ...
                  <p className="text-slate-500 font-medium font-mono text-[9px]">
                    Note: This data is stored locally in your browser (LocalStorage) and won't affect any central databases.
                  </p>
                </div>
              ...
              {gscToken && (
                <div className="bg-slate-50 border border-slate-200 p-2.5 rounded-xl flex items-center justify-between text-[11px] text-slate-600 font-bold">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      localStorage.removeItem('google_search_console_token');
                      setGscToken('');
